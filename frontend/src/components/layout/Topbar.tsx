...
import { Search, Bell, Shield, User, Command, ArrowRight, Globe } from 'lucide-react';
    ...
    <header className="flex items-center justify-between gap-4 mb-6 relative">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 hover:bg-emerald-50 text-slate-700 hover:text-[#004F3B] border border-slate-200/80 hover:border-emerald-200 text-xs font-bold transition-all shadow-2xs cursor-pointer"
          title="Return to Public Landing Page"
        >
          <Globe className="w-3.5 h-3.5 text-[#004F3B]" />
          Landing Overview
        </button>

        {/* Environment Status Badge */}
        <span className="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200/60 text-emerald-900 text-xs font-semibold">
