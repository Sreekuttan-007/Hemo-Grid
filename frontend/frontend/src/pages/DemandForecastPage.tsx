...
export const DemandForecastPage: React.FC = () => {
  ...
  const [selectedBloodGroup, setSelectedBloodGroup] = useState('O_POS');
  const [selectedComponent, setSelectedComponent] = useState('RBC');

  // Default to first shortage if none selected
  const selectedFc = shortages.find(
    (f) => f.facility_id === selectedFacilityId && f.blood_group === selectedBloodGroup && f.component === selectedComponent
  ) || shortages[0];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div>
            <h3 className="text-base font-bold text-slate-900">
              {selectedFc?.facility_name ?? 'Select Facility'} • {formatBloodGroup(selectedFc?.blood_group ?? 'O_POS')} {formatComponent(selectedFc?.component ?? 'RBC')}
            </h3>
